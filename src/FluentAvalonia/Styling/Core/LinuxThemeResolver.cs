...
internal static class LinuxThemeResolver
{
    ...
    {
        if (_config == null) TryLoadLinuxDesktopEnvironmentConfig();
            ...
            switch (_desktopEnvironment)
            {
                case DesktopEnvironment.KDE:
                {
                    var match = new Regex("^ColorScheme=(.*)$", RegexOptions.Multiline)
                        .Match(_config);
                    if (match.Success) return GetThemeFromName(match.Groups[1].Value);
    ...
    private static DesktopEnvironment GetDesktopEnvironment()
    {
        ...
        if (name == null) return DesktopEnvironment.Other;

        // Some distributions may add their own name to the variable, e.g. "ubuntu:GNOME"
        if (name.Contains("KDE")) return DesktopEnvironment.KDE;
        if (name.Contains("GNOME")) return DesktopEnvironment.GNOME;
        if (name.Contains("LXDE")) return DesktopEnvironment.LXDE;
        if (name.Contains("Cinnamon")) return DesktopEnvironment.Cinnamon;
