...
internal readonly unsafe struct LPARAM : IComparable, IEquatable<LPARAM>
{
    ...
    public static implicit operator LPARAM(byte value) => new(value);

    public static explicit operator byte(LPARAM value) => (byte)value.Value;

    public static implicit operator LPARAM(short value) => new(value);
    ...
    public static implicit operator LPARAM(sbyte value) => new(value);

    public static explicit operator sbyte(LPARAM value) => (sbyte)value.Value;

    public static implicit operator LPARAM(ushort value) => new(value);

    public static explicit operator ushort(LPARAM value) => (ushort)value.Value;

    public static explicit operator LPARAM(uint value) => new((nint)value);

    public static explicit operator uint(LPARAM value) => (uint)value.Value;

    public static explicit operator LPARAM(ulong value) => new((nint)value);

    public static explicit operator ulong(LPARAM value) => (ulong)value.Value;

    public static explicit operator LPARAM(nuint value) => new((nint)value);

    public static explicit operator nuint(LPARAM value) => (nuint)value.Value;

    public int CompareTo(object obj)
    {
        if (obj is LPARAM other) return CompareTo(other);
