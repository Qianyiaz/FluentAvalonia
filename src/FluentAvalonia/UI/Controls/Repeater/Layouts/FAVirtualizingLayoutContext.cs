...
public abstract class FAVirtualizingLayoutContext : FALayoutContext
{
    ...
    /// <summary>
    ///     Retrieves a UIElement that represents the data item in the source found at the specified index.By default, if an
    ///     element already exists, it is returned; otherwise, a new element is created.
    /// </summary>
    public Control GetOrCreateElementAt(int index) =>
        GetOrCreateElementAtCore(index, FAElementRealizationOptions.None);

    /// <summary>
    ///     Retrieves a UIElement that represents the data item in the source found at the specified index using the specified
    ///     options.
    /// </summary>
    ...
    /// <summary>
    ///     Implements the behavior for getting the return value of RecommendedAnchorIndex in a derived or custom
