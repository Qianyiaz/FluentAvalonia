...
public class FAFlowLayout : FAVirtualizingLayout, IOrientationBasedMeasures, IFlowLayoutAlgorithmDelegates
{
        ...
        FAVirtualizingLayoutContext context)
    {
        ...
        if (itemsCount > 0)
        {
            ...
            var averageLineSize = GetAverageLineInfo(availableSize, context, flowState, ref averageItemsPerLine) +
                                  LineSpacing();
            Debug.Assert(averageItemsPerLine != 0);

            var extentMajorSize = this.MajorSize(lastExtent) == 0
                ? itemsCount / averageItemsPerLine * averageLineSize
                : this.MajorSize(lastExtent);
            if (itemsCount > 0 &&
                this.MajorSize(realizationRect) > 0 &&
                DoesRealizationWindowOverlapExtent(realizationRect,
                    this.MinorMajorRect(this.MinorStart(lastExtent), this.MajorStart(lastExtent),
                        this.Minor(availableSize), extentMajorSize)))
            {
            ...
            if (firstRealized != null)
            {
                ...
                this.SetMinorSize(ref extent,
                    !double.IsInfinity(availableSizeMinor)
                        ? availableSizeMinor
                        : Math.Max(0, this.MinorEnd(lastRealizedLayoutBounds)));
            }
            else
            {
                ...
                extent = !double.IsInfinity(availableSizeMinor)
                    ? this.MinorMajorRect(0, 0, availableSizeMinor,
                        Math.Max(0, numLines * averageLineSize - lineSpacing))
                    : this.MinorMajorRect(0, 0,
                        Math.Max(0,
                            (this.Minor(flowState.SpecialElementDesiredSize) + minItemSpacing) * itemsCount -
                            minItemSpacing),
    ...
    protected internal override void InitializeForContextCore(FAVirtualizingLayoutContext context)
    {
        ...
        if (state != null) flowState = GetAsFlowState(state);
