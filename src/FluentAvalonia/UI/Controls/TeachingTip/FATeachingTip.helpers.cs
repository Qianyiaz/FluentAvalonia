...
public partial class FATeachingTip
{
    ...
    private Thickness BottomPlacementTopRightHighlightMargin(double width, double height) =>
        new(width / 2 + (TailShortSideLength() - 1f), 0, TopRightCornerRadius() - 1f, 0);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private Thickness BottomRightPlacementTopRightHighlightMargin(double width, double height) =>
        new(MinimumTipEdgeToTailEdgeMargin() + (TailLongSideLength() - 1f), 0, TopRightCornerRadius() - 1f, 0);
    ...
    private Thickness BottomPlacementTopLeftHighlightMargin(double width, double height) =>
        new(TopLeftCornerRadius() - 1, 0, width / 2 + (TailShortSideLength() - 1f), 0);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private Thickness BottomRightPlacementTopLeftHighlightMargin(double width, double height) =>
        new(TopLeftCornerRadius() - 1f, 0, width - (MinimumTipEdgeToTailEdgeMargin() + 1f), 0);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private Thickness BottomLeftPlacementTopLeftHighlightMargin(double width, double height) =>
        new(TopLeftCornerRadius() - 1f, 0, MinimumTipEdgeToTailEdgeMargin() + TailLongSideLength() - 1f, 0);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private Thickness TopEdgePlacementTopLeftHighlightMargin(double width, double height) =>
        new(TopLeftCornerRadius() - 1f, 1, TopRightCornerRadius() - 1f, 0);
    ...
    private Thickness LeftEdgePlacementTopLeftHighlightMargin(double width, double height) =>
        new(TopLeftCornerRadius() - 1f, 1, TopRightCornerRadius() - 2f, 0);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private Thickness RightEdgePlacementTopLeftHighlightMargin(double width, double height) =>
        new(TopLeftCornerRadius() - 2f, 1, TopRightCornerRadius() - 1f, 0);


    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static double UntargetedTipFarPlacementOffset(double farWindowCoordinateInCoreWindowSpace, double tipSize,
        double offset) =>
        farWindowCoordinateInCoreWindowSpace - (tipSize + s_untargetedTipWindowEdgeMargin + offset);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static double UntargetedTipCenterPlacementOffset(double nearWindowCoordinateInCoreWindowSpace,
        double farWindowCoordinateInCoreWindowSpace,
        double tipSize, double nearOffset, double farOffset) =>
        (nearWindowCoordinateInCoreWindowSpace + farWindowCoordinateInCoreWindowSpace) / 2 - tipSize / 2 + nearOffset -
        farOffset;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static double
        UntargetedTipNearPlacementOffset(double nearWindowCoordinateInCoreWindowSpace, double offset) =>
