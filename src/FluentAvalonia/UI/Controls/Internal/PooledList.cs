...
#pragma warning restore CS0419
    ...
    /// <summary>
    ///     Gets a <see cref="T:System.Span`1" /> view over the data in a list.
    ///     Items should not be added or removed from the list while the returned span is in use.
    /// </summary>
    ...
    public PooledList<TOutput> ConvertAll<TOutput>(Func<T, TOutput> converter)
    {
        ...
        for (var i = 0; i < _size; i++) list._items[i] = converter(_items[i]);
        list._size = _size;
        return list;
    }
    ...
    private Span<T> InsertSpan(int index, int count, bool clearOutput)
    {
        ...
        if (clearOutput && _clearOnFree) output.Clear();
    ...
    public int LastIndexOf(T item, int index, int count)
    {
        ...
        if (Count != 0 && count < 0) throw new ArgumentOutOfRangeException();
    ...
    public PooledList(int capacity, ClearMode clearMode, ArrayPool<T> customPool, bool sizeToCapacity)
    {
        ...
        {
            _size = capacity;
            if (clearMode != ClearMode.Never) Array.Clear(_items, 0, _size);
        }
    }
